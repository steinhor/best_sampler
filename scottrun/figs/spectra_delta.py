...
mydata = np.loadtxt('deltaspectra.dat',skiprows=0,unpack=True)
x=mydata[0]
y=mydata[1]
mydatap = np.loadtxt('deltaspectra_polemass.dat',skiprows=0,unpack=True)
xp=mydatap[0]
yp=mydatap[1]
yscale('log')
...
sangwookdata = np.loadtxt('sangwook/pTspec_Msample00_2214.dat',skiprows=1,unpack=True)
sxp=sangwookdata[0]
...
plt.savefig('spectra_delta.pdf',format='pdf')
os.system('open -a Preview spectra_delta.pdf')
#plt.show()
quit()
