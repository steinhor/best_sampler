...
double Misc::CalcDelta_FromSqWells(int ell,double mu,int nwells,double q,double *V0,double *r){
	...
	for(iwell=1;iwell<=nwells;iwell++){
		...
		if(realcheck[iwell-1]==1)
			Bessel::CalcJN_real(ell,qr,jl,nl,jlprime,nlprime);
		else
			Bessel::CalcJN_imag(ell,qr,jl,nl,jlprime,nlprime);
...
void Misc::freegascalc_onespecies(double T,double m,double &epsilon,double &P,double &dens,double &sigma2,double &dedt){
